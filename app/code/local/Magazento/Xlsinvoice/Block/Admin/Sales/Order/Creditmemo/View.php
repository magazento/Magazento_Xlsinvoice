 ...
 */
class Magazento_Xlsinvoice_Block_Admin_Sales_Order_Creditmemo_View extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        // MAGAZENTO.COM
        $xlsurl = $this->getUrl('xlsinvoice/admin_creditmemo/printxls', array('creditmemo_id'=>$this->getCreditmemo()->getId()));
        if ($this->getCreditmemo()->getId()) {
            $this->_addButton('printxls', array(
                'label'     => Mage::helper('sales')->__('Print XLS'),
                'class'     => 'save',
                'onclick'   => 'setLocation(\''.$xlsurl.'\')'
                )
            );
        }
